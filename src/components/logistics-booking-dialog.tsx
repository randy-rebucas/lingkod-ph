...
import { CalendarIcon, Loader2, Clock, MapPin, Package, Users, Home, AlertTriangle, CheckCircle, Truck, Navigation } from 'lucide-react';
...
// Logistics booking schema
const logisticsBookingSchema = z.object({
...
type LogisticsBookingFormValues = z.infer<typeof logisticsBookingSchema>;
...
type LogisticsBookingDialogProps = {
  isOpen: boolean;
...
export function LogisticsBookingDialog({ isOpen, setIsOpen, partner, onBookingConfirmed }: LogisticsBookingDialogProps) {
  const { user } = useAuth();
  ...
  const form = useForm<LogisticsBookingFormValues>({
    resolver: zodResolver(logisticsBookingSchema),
    defaultValues: {
  ...
  }, [watchedServiceType, watchedSpecialRequests, additionalStops.length, form]);

  const dynamicPrice = useMemo(() => calculateDynamicPricing(), [calculateDynamicPricing]);

  // Memoize the submit handler
  const onSubmit = useCallback(async (data: LogisticsBookingFormValues) => {
    if (!user) {
    ...
    try {
      ...
      const bookingData = {
        ...
        logisticsSubType: data.serviceType,
        ...
        // Pricing
        price: dynamicPrice,
        basePrice: serviceTypeConfig[data.serviceType].basePrice,
        additionalFees: dynamicPrice - serviceTypeConfig[data.serviceType].basePrice,
        
        // Service details
        serviceName: serviceTypeConfig[data.serviceType].label,
        ...
        statusHistory: [{
          ...
          note: 'Logistics booking created - waiting for partner acceptance',
        }],
        notifications: {
        ...
        // Commission tracking
        commission: {
          partnerCommission: 0.15, // 15% for partner
          localProCommission: 0.05, // 5% for LocalPro
          providerEarnings: dynamicPrice * 0.8, // 80% for provider
        },
      ...
      const newBookingRef = await addDoc(collection(getDb(), 'logisticsBookings'), bookingData);
      ...
      await addDoc(collection(getDb(), 'notifications'), {
        ...
        message: `New ${data.serviceType} booking from ${user.displayName}`,
        data: {
      ...
      toast({
        title: "Logistics Booking Created",
        description: `Your ${data.serviceType} booking has been created. The partner will be notified.`,
      });
      
      router.push(`/bookings/${newBookingRef.id}/tracking`);
  ...
  }, [user, partner, onBookingConfirmed, router, toast, t, dynamicPrice, additionalStops]);
        ...
        return (
          <div className="space-y-6">
            <div className="space-y-4">
              <Label className="text-lg font-semibold">Logistics Service Type</Label>
                      ...
                      <CardContent className="p-4">
                        <div className="flex items-center justify-between">
                          ...
                          <div className="text-right">
                            ...
                            <p className="font-bold text-primary">₱{config.basePrice}</p>
                          </div>
                        </div>
                      </CardContent>
                    </Card>
            ...
            <div className="space-y-4">
              ...
              <div className="space-y-2">
                <div className="flex justify-between text-lg">
                  ...
                  <span>₱{serviceTypeConfig[watchedServiceType].basePrice.toFixed(2)}</span>
                </div>
                {dynamicPrice > serviceTypeConfig[watchedServiceType].basePrice && (
                  <div className="flex justify-between text-sm text-muted-foreground">
                    <span>Additional Fees:</span>
                    <span>₱{(dynamicPrice - serviceTypeConfig[watchedServiceType].basePrice).toFixed(2)}</span>
                  </div>
                )}
                <Separator />
  ...
  return (
    <Dialog open={isOpen} onOpenChange={setIsOpen}>
      <DialogContent className="sm:max-w-2xl max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle>Book Logistics Service with {partner.displayName}</DialogTitle>
          <DialogDescription>
            Create a logistics booking. Step {currentStep} of 4.
          </DialogDescription>
        </DialogHeader>
            ...
            <div className="flex justify-between pt-4">
              ...
              {currentStep < 4 ? (
                ...
                <Button type="submit" disabled={isSaving}>
                  ...
                  {isSaving ? 'Creating Booking...' : 'Create Logistics Booking'}
                </Button>
              )}
            </div>
          </form>
