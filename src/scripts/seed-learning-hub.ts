...
import { 
  ...
  type Category,
  type Article
} from '../lib/firebase/learning-hub';
...
async function main() {
  console.log('🚀 Starting Learning Hub seed process...');
  ...
  try {
    ...
    for (const categoryData of seedData.categories) {
      const category = await CategoryModel.create(categoryData as any);
    ...
    console.log('📁 Backup data saved to: src/data/learning-hub-seed.json');
    console.log('');
    console.log('📊 Seed data includes:');
    console.log(`   - ${articles.length} authors`);
